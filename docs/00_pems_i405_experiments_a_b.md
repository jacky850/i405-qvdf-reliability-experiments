# PeMS I-405 South: Experiments A and B (method detail)

[Back to the summary README](../README.md)

This page keeps the full step-by-step method for the PeMS prototype. Commands are run from the repository root.

> Memo relabel (Section 5): in Experiment A the regressor $D/C$ below is the *capacity-equivalent congested-flow duration* $X_E=\int_E q\,dt/C$ in hours, so $f_d$ carries units of $\mathrm{h}^{1-n}$, not hours. The fit is an internal consistency check because $P$ and $X_E$ share the episode endpoints. Experiment B part 3 ($\alpha=0.278$, $\beta=1.772$, $\sigma_{\ln(D/C)}=0.0376$) is a provisional sanity check for this corridor only.
...
The same threshold-sensitivity structure has also been run on 23 weekdays of
NVTA/RITIS speed data for I-95 northbound AM. The NVTA version first reconstructs
flow with the existing inverse S3 model, then recalculates $P$ and $D/C$ and
refits $f_d,n$ at every threshold. The GP fit uses 276 common-support TMC-days;
the northbound managed lane is reported separately and is not fitted because it
has only two common congested TMC-days.

See [NVTA Experiment A: I-95 Northbound AM](03_nvta_experiment_a_i95_nb_am.md).

The inherited free-flow-speed and capacity priors are reviewed in
[I-95 Northbound Free-Flow Speed and Capacity Audit](04_nvta_i95_free_flow_capacity_audit.md).

![NVTA I-95 Northbound AM threshold sensitivity](../figures/nvta_a_threshold_parameter_sensitivity.png)

The audited-parameter v2 run replaces the facility-wide constants with each
TMC's weekday off-peak speed P85 and its matched Cube AM network capacity. The
legacy outputs remain available for comparison. See
[NVTA Experiment A v2: Audited TMC Parameters](05_nvta_experiment_a_i95_nb_am_empirical_parameters.md).

![NVTA I-95 Northbound AM audited-parameter sensitivity](../figures/nvta_a_v2_threshold_parameter_sensitivity.png)

## NVTA B1 severe-congestion cutoff test

The B1 test compares the current $0.50v_f$ severe-congestion cutoff with the
proposed $0.25v_f$ cutoff using the audited TMC parameters. It reports daily
congested link-mile-hours separately for GP and managed lanes and for the
combined corridor. See
[NVTA B1 Speed-Cutoff Test](06_nvta_b1_speed_cutoff_test.md).

![NVTA I-95 Northbound AM B1 cutoff sensitivity](../figures/nvta_b1_speed_cutoff_sensitivity.png)
